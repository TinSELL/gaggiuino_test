*__这是运行 (RUN) 配置文件，应在萃取前切换至此。请先让机器在准备 (PREP) 配置文件下稳定温度。__*

Flimsy 低接触 (Low Contact)（以及超低接触 Ultra Low Contact）是 Turbo 风格的配置文件，旨在突出干净度和酸质，同时尽量减少干涩感和刺鼻感。它们通过低接触时间、不断降低的低温以及较长的粉液比的结合来实现这一点。

特别适合风味精致的浅烘焙和经过高度处理的咖啡豆（如共发酵豆），这些豆子在杯测和手冲方法中呈现的理想风味，在更传统的或更高温度的浓缩咖啡配置文件中往往会被混淆、淹没或被过度放大。整体的“强度”可能比其他配置文件略低一些，因此如果你发现你的萃取结果尝起来有点平淡，你的豆子可能适合使用其他不同的配置文件，比如 Zer0、IUIUIU 或 Extractamundo Dos。

这些配置文件设计用于高萃取率粉碗，或带有底部滤纸的传统粉碗（如 VST），粉液比在 1:2.5 到 1:3 之间。你可能需要比其他诸如 extractamundo dos 等 turbo 配置文件稍微粗一点的研磨度。LC 的目标时间约为 13-16 秒，ULC 的目标时间约为 12-15 秒，当然，请根据口味进行微调！

分离出 PREP 和 RUN 两个配置文件的目的是为浸泡阶段设置一个较高的初始温度，但在整个萃取过程中，让流入锅炉的水来冷却冲煮温度。

LC（低接触）是主要的 RUN 配置文件，其流量和压力有限制，比 ULC（超低接触）的容错率稍微高一点。ULC 则将这一理念进一步推向极致，如果你已经完全掌握了 LC，它非常值得一试。

通常来说，同时使用 PREP 和 RUN 配置文件会得到更平衡的结果，然而 Gaggiuino Discord 上的一些用户发现，跳过 PREP 配置文件并将 RUN 设置为中等温度（72-73°C）可以制作出果汁感非常强烈的咖啡，代价是强度和传统浓缩咖啡风味的进一步降低。这似乎特别适合那些非常容易萃取出水果风味的共发酵豆以及类似品种。

LC 和 ULC 均基于 LongPVO 为 Decent 意式浓缩咖啡机制作的同名配置文件。

---
*Original text:*
*__This is the RUN profile, which should be changed to just before the shot is pulled. Have the machine stabilise temperature on the PREP profile first.__*
