...
这个配置文件试图在 Gaggiuino 上模拟这种行为，方法是设置多个递增的压力阶段，将“泵水量”作为停止条件，从 6 bar 开始递减。
...
最后一个阶段是从 3 bar 开始的压力下降，在 10 秒后或杯中达到 10 克时停止。

*主要萃取（阶段 4 及以后）：*

所有阶段的“压力”和“泵水量”的组合定义了模拟拉杆机的“弹簧行为”和“气缸容量”。

萃取从上升到 6 bar 开始，并持续下降。
在达到 4 bar 后的输出重量应该在 60 克左右。
主要阶段的流量被限制在 4 ml/s，以防止“喷涌 (gushers)”。
...
This profile tries to emulate this behaviour with Gaggiuino by setting up multiple incremental pressure phases with "water pumped" as stop conditions, declining from 6 bar.
...
The last phase is a pressure drop starting from 3 bar, stopping either after 10 seconds or 10 grams in the cup.
...
Extraction starts with a rise to 6 bar and continues to decline.
The output weight after reaching 4 bar should be about 60 grams.
The flow in the main phases is limited to 4 ml/s to prevent "gushers".
